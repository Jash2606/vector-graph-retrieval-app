...
class DocumentController:
    ...
    def update_document(self, node_id: str, update_data: NodeUpdate) -> Dict[str, Any]:
        ...
        if update_data.regen_embedding and node:
            text_to_embed = update_data.text if update_data.text else node.get('text')
            if text_to_embed and node.get('vector_id') is not None:
                ...
                self.neo4j_repo.delete_relationships(node_id, ['RELATED_TO', 'MENTIONS'])
