...
class SearchController:
    ...
    def __init__(self, neo4j_repo: Neo4jRepository, vector_repo: VectorRepository):
    ...
    def graph_search(self, start_id: str, depth: int, relationship_types: List[str] = None) -> Dict:
        """Perform pure graph traversal"""
        try:
            return self.neo4j_repo.graph_search(start_id, depth, relationship_types)
        except Exception as e:
            logger.error(f"Graph search failed: {str(e)}", exc_info=True)
            raise SearchError(f"Graph search failed: {str(e)}")
    
    def hybrid_search(self, query_text: str, vector_weight: float, graph_weight: float,
                     top_k: int, graph_depth: int, query_embedding: List[float] = None
        ...
        try:
            ...
            if query_embedding:
                ...
                distances, indices = self.vector_repo.search(query_vector, top_k * 3)
            ...
            if query_entities:
                entity_docs = self.neo4j_repo.find_entity_documents(query_entities, limit=50)
            ...
            connectivity_scores = self.neo4j_repo.get_connectivity_scores(candidate_ids)
