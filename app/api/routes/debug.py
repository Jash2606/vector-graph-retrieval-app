...
def get_all_documents(neo4j_repo: Neo4jRepository = Depends(get_neo4j_repository)) -> List[Dict[str, Any]]:
...
@router.get("/entities")
def get_all_entities(neo4j_repo: Neo4jRepository = Depends(get_neo4j_repository)) -> List[Dict[str, Any]]:
...
def get_faiss_info(vector_repo: VectorRepository = Depends(get_vector_repository)) -> Dict[str, Any]:
