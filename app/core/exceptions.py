...
class DocumentNotFoundError(BaseAPIException):
    ...
    def __init__(self, doc_id: str):
        super().__init__(f"Document with ID '{doc_id}' not found", status_code=404)
        self.doc_id = doc_id
...
class NodeNotFoundError(BaseAPIException):
    ...
    def __init__(self, node_id: str):
        super().__init__(f"Node with ID '{node_id}' not found", status_code=404)
...
class IngestionError(BaseAPIException):
    ...
    def __init__(self, reason: str):
        super().__init__(f"Document ingestion failed: {reason}", status_code=500)
...
class ValidationError(BaseAPIException):
    ...
    def __init__(self, field: str, reason: str):
        super().__init__(f"Validation error for '{field}': {reason}", status_code=422)
