...
class Neo4jRepository(BaseRepository):
    ...
    def create_entity_node(self, name: str, entity_type: str,
    ...
    def graph_search(self, start_id: str, depth: int,
                     relationship_types: Optional[List[str]] = None) -> Dict:
        """Fetch nodes and relationships within depth"""
        rel_pattern = ""
        if relationship_types:
            safe_types = [
                t for t in relationship_types if t.isalnum() or "_" in t]
            if safe_types:
            ...
            for record in res:
                ...
                def get_node_id(node):
                    return node.get('id') or (
                        node.element_id if hasattr(
                            node, 'element_id') else str(
                            node.id))
                ...
                if edge_key not in seen_edges:
                    ...
                    source_text = source.get('text', '')[:100] if source.get(
                        'text') else source.get('name', '')
                    target_text = target.get('text', '')[:100] if target.get(
                        'text') else target.get('name', '')
    ...
    def get_connectivity_scores(
            self, candidate_ids: List[str]) -> Dict[str, float]:
        ...
        with self.driver.get_session() as session:
            ...
            for record in res:
                connectivity_scores[record["cid"]
                                    ] = record["adj_weight"] or 0.0
